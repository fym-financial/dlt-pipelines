...
- `typed.heartland_inforced_policy_latest`: newest observed version for each `pol_no`, `agt_code`, and `writing_split` combination. It includes `previous_hnl_status` and `previous_hnl_status_date`; the date records when the current status was first observed after changing from the previous distinct status.
